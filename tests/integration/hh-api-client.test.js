...
test("hh api client: sendMessage uses form-encoded message body for HH negotiations API", async () => {
  ...
  const client = new HhApiClient({
    ...
    fetchImpl: async (url, init) => {
      calls.push({ url: String(url), init });
      return jsonResponse(201, { id: "msg-001" });
    }
  });

  const result = await client.sendMessage("neg-001", "Напишите мне в Telegram @kobzevvv");
  ...
  assert.equal(calls[0].init.body, "message=%D0%9D%D0%B0%D0%BF%D0%B8%D1%88%D0%B8%D1%82%D0%B5+%D0%BC%D0%BD%D0%B5+%D0%B2+Telegram+%40kobzevvv");
});
