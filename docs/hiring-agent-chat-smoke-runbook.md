...
5. `POST /api/chat`  
Проверяет JSON-контракт ответа (`reply.kind` и базовую структуру payload для выбранного сценария).

6. (Опционально) `GET /api/artifacts/:id`  
Проверяет чтение артефакта только для сценариев, которые его реально возвращают.
...
Проверка, что ключевой сценарий чата возвращает валидный `reply.kind` и ожидаемую структуру ответа.
...
- для сценариев с артефактным контрактом есть `artifact.id` и артефакт читается;
