...
| 1 | `llm_generate` | Составить варианты плана коммуникации | На основании application_steps генерирует 3–4 варианта последовательности шагов. |
| 2 | `display` | Показать варианты, предложить утвердить | Показать таблицей + кнопки «Утвердить / Уточнить». |
| 3 | `llm_generate` | Сгенерировать примеры сообщений кандидату | 3 реалистичных первых сообщения, HTML. Использует script из application_steps и FAQ. |
...
| 2 | `llm_extract` | Обработать критерий | Определяет: точный SQL-фильтр или нечёткий поиск с порогом. Возвращает {type, sql?, threshold?, description}. |
| 3 | `decision` | Уточнить логику у рекрутера | Объясняет как планирует выбирать, предлагает подтвердить порог. |
| 4 | `llm_generate` | Сгенерировать сообщения и репорт | Список кандидатов с персонализированными сообщениями. |
