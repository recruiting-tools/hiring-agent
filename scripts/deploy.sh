...
PROJECT="${GCP_PROJECT_ID:-${GOOGLE_CLOUD_PROJECT:-${GCLOUD_PROJECT:-${CLOUDSDK_PROJECT:-${CLOUDSDK_CORE_PROJECT:-}}}}}"
REGION="${GCP_REGION:-europe-west1}"
if [ -z "$PROJECT" ]; then
  echo "ERROR: GCP project id is required (checked GCP_PROJECT_ID, GOOGLE_CLOUD_PROJECT, GCLOUD_PROJECT, CLOUDSDK_PROJECT, CLOUDSDK_CORE_PROJECT)"
  exit 1
fi
...
if [ -n "${HH_REDIRECT_URI:-}" ]; then
  ENV_VARS="$ENV_VARS,HH_REDIRECT_URI=$HH_REDIRECT_URI"
fi
...
if [ "$SECRET_PROFILE" = "sandbox" ]; then
  ...
  SECRETS="GEMINI_API_KEY=GEMINI_API_KEY:latest,$DB_SECRET_ENV=$DB_SECRET_NAME:latest,SESSION_SECRET=SESSION_SECRET:latest,HH_CLIENT_ID=HH_CLIENT_ID:latest,HH_CLIENT_SECRET=HH_CLIENT_SECRET:latest,TELEGRAM_BOT_TOKEN=TELEGRAM_BOT_TOKEN:latest,INTERNAL_API_TOKEN=INTERNAL_API_TOKEN:latest,HH_VACANCY_JOB_MAP=HH_VACANCY_JOB_MAP:latest"
fi
...
if [ $exit_code -eq 0 ]; then
  ...
  if [ "$IS_SANDBOX" = true ]; then
    ...
    if [ -z "$PUBLIC_URL" ]; then
      echo "Skipping post-deploy smoke: DEPLOY_PUBLIC_URL is not set."
    else
